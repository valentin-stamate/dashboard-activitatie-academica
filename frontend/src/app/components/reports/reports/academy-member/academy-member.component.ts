...
import {AcademyMember} from "../../reports.util";
import {UIMessages, UtilService} from "../../../../service/util.service";

@Component({
  selector: 'app-academy-member',
  templateUrl: './academy-member.component.html',
  styleUrls: ['./academy-member.component.scss']
})
export class AcademyMemberComponent implements OnInit {

  @Input()
  form: AcademyMember = new AcademyMember();
  ...
  @Input() updateEmitter!: EventEmitter<AcademyMember>;
  ...
  onUpdateEvent(item: AcademyMember) {
    this.editMode = true;
    this.form = item;
  }

  onCancel() {
    this.form = new AcademyMember();
    this.editMode = false;
  }
  ...
  onSubmit(event: Event) {
    ...
    if (this.editMode) {
      axios.patch(`${RestEndpoints.ACADEMY_MEMBER}/${this.form.id}`, this.form, this.config)
        .then(res => {
          this.form = new AcademyMember();
    ...
    axios.post(RestEndpoints.ACADEMY_MEMBER, this.form, this.config)
      .then(res => {
        this.form = new AcademyMember();
