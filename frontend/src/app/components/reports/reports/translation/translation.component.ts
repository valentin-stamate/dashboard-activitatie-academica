...
import {ReportsUtil, Translation} from "../../reports.util";
...
export class TranslationComponent implements OnInit {
  ...
  edition = ReportsUtil.NationalInternationalFem;

  @Input()
  form: Translation = new Translation();
  ...
  @Input() updateEmitter!: EventEmitter<Translation>;
  ...
  onUpdateEvent(item: Translation) {
    this.editMode = true;
    this.form = item;
  }

  onCancel() {
    this.form = new Translation();
    this.editMode = false;
  }
  ...
  onSubmit(event: Event) {
    ...
    if (this.editMode) {
      axios.patch(`${RestEndpoints.TRANSLATION}/${this.form.id}`, this.form, this.config)
        .then(res => {
          this.form = new Translation();
    ...
    axios.post(RestEndpoints.TRANSLATION, this.form, this.config)
      .then(res => {
        this.form = new Translation();
