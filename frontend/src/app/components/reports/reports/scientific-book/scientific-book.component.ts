...
import {ReportsUtil, ScientificBook} from "../../reports.util";
...
@Component({
  selector: 'app-scientific-book',
  templateUrl: './scientific-book.component.html',
  styleUrls: ['./scientific-book.component.scss']
})
export class ScientificBookComponent implements OnInit {
  hierarchyDomains = ReportsUtil.HierarchyDomains;
  publicationType = ReportsUtil.NationalInternationalFem;

  @Input()
  form: ScientificBook = new ScientificBook();
  ...
  @Input() updateEmitter!: EventEmitter<ScientificBook>;
  ...
  onUpdateEvent(item: ScientificBook) {
    this.editMode = true;
    this.form = item;
  }

  onCancel() {
    this.form = new ScientificBook();
    this.editMode = false;
  }
  ...
  onSubmit(event: Event) {
    ...
    if (this.editMode) {
      axios.patch(`${RestEndpoints.SCIENTIFIC_BOOK}/${this.form.id}`, this.form, this.config)
        .then(res => {
          this.form = new ScientificBook();
    ...
    axios.post(RestEndpoints.SCIENTIFIC_BOOK, this.form, this.config)
      .then(res => {
        this.form = new ScientificBook();
