...
import {Patent, ReportsUtil} from "../../reports.util";
...
export class PatentComponent implements OnInit {
  patentType = ReportsUtil.NationalInternationalMasc;

  @Input()
  form: Patent = new Patent();
  ...
  @Input() updateEmitter!: EventEmitter<Patent>;
  ...
  onUpdateEvent(item: Patent) {
    this.editMode = true;
    this.form = item;
  }

  onCancel() {
    this.form = new Patent();
    this.editMode = false;
  }
  ...
  onSubmit(event: Event) {
    ...
    if (this.editMode) {
      axios.patch(`${RestEndpoints.PATENT}/${this.form.id}`, this.form, this.config)
        .then(res => {
          this.form = new Patent();
    ...
    axios.post(RestEndpoints.PATENT, this.form, this.config)
      .then(res => {
        this.form = new Patent();
